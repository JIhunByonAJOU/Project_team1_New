...
class ConsoleWriter(StatsWriter):
    ...
    def write_stats(
        ...
        if "Environment/Cumulative Reward" in values:
            ...
            self._append_drt_episode_metrics(log_info, values)
    ...
    def _append_drt_episode_metrics(
        self, log_info: List[str], values: Dict[str, StatsSummary]
    ) -> None:
        metric_formats = {
            "DRT/EpisodeTravelDistanceMeters": "Distance: {value:0.1f} m",
            "DRT/EpisodeAverageWaitSeconds": "Avg Wait: {value:0.1f} s",
            "DRT/EpisodeAverageRideSeconds": "Avg Ride: {value:0.1f} s",
            "DRT/EpisodeServiceRate": "Service Rate: {value:0.3f}",
            "DRT/EpisodeCompletedPassengers": "Completed: {value:0.0f}",
        }

        for key, template in metric_formats.items():
            if key in values:
                log_info.append(template.format(value=values[key].aggregated_value))
